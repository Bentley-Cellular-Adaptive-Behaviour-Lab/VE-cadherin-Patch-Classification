...
% % picks object thresholds blindly hence potential overwriting problems
% PickThreshes([1],lsmlist2)     
% disp(' copy the *_StdThreshV3.mat in a folder called HandThresholds');
% disp(' press any key to continue');
% pause
% AutoClass([1],lsmlist2)     % this does classification based on blind object thresholds

% we mainly did visible threholds for the later projects
% so *** USE THESE ONES *****
% PickThreshesVis([1],lsmlist2)     % picks object thresholds visibly via kb/
AutoClass([1],lsmlist2,3)     % end option 3 classifies based on visble object thresholds

% this does the automatic results stuff
% TextureResultsHand(1,lsmlist6)
...
%  ShowClassifyPatches(lsmlist3,[1:9],3,'coloc');%[2:3 7:14])  % this shows the difference when just dmso
...
%     fnout=['Classified/' fn(1:end-4) '_sl' int2str(sl) '_TypeV3.mat'];
    fnout=['Classified/' fn(1:end-4) '_sl' int2str(sl) '_TypeV2.mat'];
    load(fno)
    if(~isequal(mask,-1))
        clear ptype strens
        load(fnout)
...
function[clustf]=GetDataToCluster(inds,lsmlist)
fn=char(lsmlist(1));
...
for k=inds
    fn=char(lsmlist(k));
    smallf=[fn(1:end-4) 'SliceDataSF_HandSmall.mat'];
    if(~isfile(smallf))
        tempSmallFiles(k,lsmlist);
    end
    load(smallf)
...
while 1
    ...
    inp=input(['enter threshold, currently ' num2str(thresh) '. -1 if bad; Return iiiif ok:  ']);
    if(isempty(inp))
        break;
    elseif(inp<0)
        %         keyboard;
