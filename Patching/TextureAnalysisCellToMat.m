...
lsmlist6={'VEGFR1inj-1-z63x-1.lsm'};
...
% PickThreshes([1 4 5],lsmlist3)     % this does picking thresholds
...
% this generates does the automatic results stuff
...
for sl=1:nl
    ...
    elseif(max(mask(:))==0)
        disp(['the mask in file ' fno ' is empty; delete it and redo?'])
        keyboard
    else
        hereiam=[ sl nl]
        im=tiffread29(fn,2*sl-1);
        rim=MyCell2mat(im,vecadCh);
        vs=rim(:).*mask(:);
        vs=vs(vs>0);
        l=length(vs);
        if((c+l)>nl*1e5)
...
%                     subplot(1,2,2)
                    ...
                    elseif(ismember(inp,[1:5]))
                        ptype(r)=inp;
                        if(ismember(inp,[1:3]))
                            strens(r)=input('enter strength, 1=high, 2=med,  3=low:  ');
...
for sl=1:nl
    ...
    if(~isfile(fno))
        im=tiffread29(fn,2*sl-1);
        rim=MyCell2mat(im,chan);
        figure(1)
        if(nargin<3)
            [thresh,mask]=getMaskSl(rim,thresh);
        else
            vim=MyCell2mat(im,vchan);
...
function[rim] = MyCell2mat(im,chan)
if(iscell(im.data))
        ...
        if(isequal(mask,-1))
            ...
            for j=goods
                ...
                if(da.nover==0)
                    ...
                    da.medsOver=prctile(vec,[50 75 95]);
                end
                if 1
...
v_im=v.*mask(:);
out.g=prctile(v_im(v_im>0),[50 25 75])';
out.g_std=prctile(vsim(vsim>0),[50 25 75])';

noObj=(1-t2im).*mask;
vnoObj=noObj(:).*v;
out.backG=prctile(vnoObj(vnoObj>0),[50 25 75])';

noObjBig=(1-imdilate(t2im,nh)).*mask;
vnoObj=noObjBig(:).*sim(:);
out.backStd=prctile(vnoObj(vnoObj>0),[50 25 75])';

noObj_s=(1-s2im).*mask;
vnoObj_s=noObj_s(:).*vsim;
vnoObj_v=noObj_s(:).*v;
out.backStd_s=prctile(vnoObj_s(vnoObj_s>0),[50 25 75])';
out.backG_s=prctile(vnoObj_v(vnoObj_v>0),[50 25 75])';
