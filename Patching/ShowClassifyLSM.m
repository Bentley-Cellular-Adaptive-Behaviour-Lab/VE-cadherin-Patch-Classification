...
lsmlist6={'T8_4.lsm'};%{'1_front.lsm';'1_internal.lsm';'2_front.lsm';'2_internal.lsm'};

lsmlist7={'2_T5.lsm';'2_T7.lsm';'3_T7.lsm'}; 

lsmlist8={'72-9 VEC685 488 VEC 555 z63x-1';'72-9 VEC685 488 VEC 555 z63x-2';...
    '72-9 VEC685 488 VEC 555 z63x-3';'72-9 VEC685 488 VEC 555 z63x-4';...
    '72-9 VEC685 488 VEC 555 z63x-5';'72-9 VEC685 488 VEC 555 z63x-6'}; 

TextureResultsHandMin(1,lsmlist6,0)           % use option 0 if the hand file is SF_Hand.mat i.e. 'blind'
% TextureResultsHandMin(1:3,lsmlist7,1)           % use option 1 for SFVis_Hand.mat
% PlotAllRedData(lsmlist3,1:9);
% AssignPhosphoDataToPatch(lsmlist8,1:6)
return

function AssignPhosphoDataToPatch(lsms,inds,opt)

if(nargin<3); 
    opt=0;
end;
mea=[];
load RatioDataPerSlice2.mat
for i=inds
    fn=char(lsms(i));
    if(opt==2)  % vecadretina stuff
    end
%     mea=[mea;TextureSlicesPhospho(fn,i)];
    mea=[mea;TextureSlicesPhospho(fn,i,ratioPerSliceCellArray(i))];
end
csvwrite('out_test.csv',mea);
x=[-3:-1 1:3];
errorbar(x,mean(mea),std(mea)),
title('mean and sd phospho in each class across samples'),
axis tight


function[mea]=TextureSlicesPhospho(fn,ind,ratioImgCellArray)

load([fn '_Maxes.mat'])
nl=length(maxes);
% load([fn(1:end-4) '_MaxesRed.mat'])
thr=0.15;%round(mamax/15);
% load RatioData.mat; % load RatioDataPerSlice.mat

rimall=cell2mat(ratioImgCellArray);
for rthresh=thr%15%[5:5:25]
    outf=[fn 'PatchedPhospoData.mat'];
    dat=[];
    for sl=1:nl

        clear mask ptype strens patches
        fno=[fn '_sl' int2str(sl) '_Mask.mat'];
        hereiam=[sl nl]
        load(fno)
        if(isequal(mask,-1))
        else
%             patches=TileImage(mask,100,100);
            ar=[patches.area];
%             load([fn '_sl' int2str(sl) '_Phospho.mat']);
%             rim=cell2mat(ratioImgCellArray(sl));
%             rim=cell2mat(ratioImgCellArray(ind,sl)); %???
            rim=rimall(:,:,sl);
            rthr=0.15*max(rim(:));
...
%             tim=double((rim>rthresh).*mask);
            ...
            for j=goods
                ...
                if(da.area==0)
                    ...
                    da.mean=mean(vec);
                    da.meds=MyPrctile(vec,[50 75 95]);
                end
                da.vec=vec';
                ...
                if(da.nover==0)
                    ...
                    da.medsOver=MyPrctile(vec,[50 75 95]);
                end
                if 1
%                     load([fn(1:end-4) '_sl' int2str(sl) '_TypeV2.mat']);
                    load([fn '_sl' int2str(sl) '_TypeV2.mat']);
                    da.ptype=ptype(j);
                    da.stren=strens(j);
...
%     x=[-3:-1 9 1:3 5:7];
    ...
    if 0
        ...
        m=2;col=2;
%     m=3;n=2;
    subplot(m,col,1),bar(x,n),title('# patches in each class'),axis tight
%     subplot(m,col,2),bar(x,[mea;med]'),title('mean/median (blue/red) phospho in each class'),axis tight
    subplot(m,col,2),BarErrorBar(x',mea',sd'),title('mean phospho in each class (bar=s.d)'),axis tight
    subplot(m,col,3),bar(x,pco),title('% phospho over thresh in each class'),axis tight
    subplot(m,col,4),bar(x,[meao;medo]'),title('mean/median (blue/red) phospho over thresh in each class'),axis tight
%     subplot(m,col,6),bar(x,[pc75;pc95]'),title('75/95 phospho in each class'),axis tight
%     subplot(m,col,5),bar(x,[pc75o;pc95o]'),title('75/95 phospho over thresh in each class'),axis tight
    end
end


function TextureResultsHandMin(indsi,lsmlist,opt)
...
% this is the key bit which goes to get all the data
[nd,allvs,allcs,infs]=GetDataToCluster(indsi,lsmlist,opt);
...
%     [cs,ncla,empts]=GetCsForClustering(vs,clasl,clas,indsclas);
    ...
    if(i>0)
        ...
        ncla=nd(i).ncla;
        str='subplot(7,2,i)';
        inf=deblank(infs(i,:));%char(lsmlist(i));
    else
        vs=allvs;%(:,1:5);
        cls=allcs;
        ...
        cs=csall;%(:,1:5);
        str='subplot(1,1,1)';
        inf='combined data';%char(lsmlist(i));
    end

% get rid of the ones with no objects at all: this needs checking as I;m
% not sure allcs is right: it's not the same as csall...
    %     nzs=sum(vs(:,1:3),2)~=0;
%     nzcla=allcs(nzs);
%     vs=vs(nzs,:);  
%     allcs=allcs(nzs);
    
    % pre-process data: needs checing that works for non-summary
    if(~Kate)
        [pv,ps]=mapstd(vs');
        pv=pv';
    ...
    % process all data into the means, sd's etc into the different
    % classes that they are in
%     js=[1:6 10];             % this is active/inhibited plus empty 
    js=[1:6];                % this is just active/inhibited
    clear s mv;
    for j=js
        is=(cls==clas(j));
        s(j,:)=std(vs(is,:),0,1);
...
%         iqv(j,:)=MyIqr(vs(is,:),1);        
    ...
    for j=js
        is=((cls==clas(j))&(sum(vs(:,13:14),2)~=0));
        s(j,17)=std(vs(is,4));
    ...
    % get the totals across all patches/all patches that are active/inhib
    mtotv=mean(vs,1);
    stotv=std(vs,1);
    ntotv=size(vs,1);
    isallc=ismember(cls,clas(1:6));
    mtotallc=mean(vs(isallc,:),1);
    stotallc=std(vs(isallc,:),1);
    nallc=sum(isallc);
...
% MAIN PLOT: this plots selected variables of the above
% here i'm doing 4 of them, 13, 14 4 and 5
    figure(i+1)
    cols=['b';'r';'k';'g';'k';'g'];
    ...
    for j=1:length(i_pl)
        subplot(2,2,j)
        BarErrorBar(x(js),mv(js,i_pl(j)),s(js,i_pl(j)));axis tight
%         errorbar(x,mv(js,i_pl(j)),s(js,i_pl(j)),cols(j));axis tight
        title(char(strs(i_pl(j))))
    end
    xlabel(['data from ' inf])

    if(i==0)
        ninclass=ncla(1:6);
        vals=vs(isallc,i_pl);
        save temp_RndDat vals ninclass
    end
    
    % this bit writes the data to an excel file
    % if you want to write more data simply make a new i_pl here eg
    % i_pl=1:14  % will write all the variables
    if(i>0)
        outf=[inf(1:end-4) '.xls'];
        xlswrite(outf,{'Data from',inf},1,'A1');
    else
        outf='Total.xls';
        xf=[];
        for k=1:size(infs,1)
        ...
        xlswrite(outf,{'Data from',xf},1,'A1');
    end
    % sort out the ranges of the xl to plot in
    sp=6;
    sp2=sp+length(js);
    sp3=sp2+length(js)+3;
%     ep=length(js)+sp-1;
%     ep2=sp2+length(js)-1;
    ...
    % write the data
%     xlswrite(outf,{'order:','mean','s.d.'},1,'A3');
%     xlswrite(outf,strs(i_pl)',1,'A5');
%     outx=[mv(js,i_pl);zeros(size(i_pl))*NaN;s(js,i_pl)];
%     xlswrite(outf,outx,1,['A' int2str(sp)]);
    
    % write the data and totals 
%     keyboard % add in the number of patch
    xlswrite(outf,{'order:','mean','s.d.'},1,'A3');
    xlswrite(outf,strs([19 i_pl 20])',1,'A5');
    means=[[clas(js)';100;-100] ...
        [mv(js,i_pl); mtotv(i_pl); mtotallc(i_pl)] ...
        [ncla(js)';ntotv;nallc]];
    stds=[[clas(js)';100;-100] ...
        [s(js,i_pl); stotv(i_pl); stotallc(i_pl)] ...
        [ncla(js)';ntotv;nallc]];
    outx=[means; zeros(1,size(stds,2))*NaN; stds];
    xlswrite(outf,outx,1,['A' int2str(sp)]);    

    xlswrite(outf,{'All Patches'},1,['A' int2str(sp2)]); 
    xlswrite(outf,{'All active/inhib'},1,['A' int2str(sp2+1)]); 
    xlswrite(outf,{'All Patches'},1,['A' int2str(sp3)]); 
    xlswrite(outf,{'All active/inhib'},1,['A' int2str(sp3+1)]); 

%     xlswrite(outf,mv(js,i_pl),1,['A' int2str(sp)]);    
%     xlswrite(outf,s(js,i_pl),1,['A' int2str(sp2)]);
    
% this bit plots all the values above as bars w error bars
%^ for each of the values in itouse for each of the classes
    str='subplot(5,4,pl)';
%     str='subplot(4,4,pl)';
    itouse=1:18;
    ...
    for pl=itouse
        ...
        BarErrorBar(x(js),mv(js,pl),s(js,pl));axis tight
        title(char(strs(pl)))
    end
    xlabel(['data from ' inf])

% % this does the same as above but uses the normalised values
if(~Kate)
    for pl=1:14
     figure(19)
        eval(str)
...
%     % more eyeballing: plots the distributions of the 
%     % various properties for each class: should show if there's a decent
...
%         end
    ...
    % this bit does a 2d scatter plot of 2 properties to see if there's any
...
%         for j=[1:6]%6 10]
%             is=(cls==clas(j));
%             plot(vs(is,i_pl(1)),vs(is,i_pl(2)),[cst(j,1) 'x'])
%             hold on
%         end
%         hold off
%                  subplot(1,2,2)
%         for j=[1:6]%6 10]
%             is=(cls==clas(j));
%             plot(vs(is,i_pl(3)),vs(is,i_pl(4)),[cst(j,1) 'x'])
%             hold on
%         end
%         hold off
...
%   % this does all the classifiying
%     nzs=sum(vs(:,1:3),2)~=0;
%     nzcla=cls(nzs);
%     nzvs=vs(nzs,:);
...
function[nd,allvs,allcs,infs]=GetDataToCluster(inds,lsmlist,opt)
% fn=char(lsmlist(1));
% clustf=[fn(1:4) 'ClusterSliceDataSF_HandResAutoW0.mat'];
% if(isfile(clustf))
%     return;
% end
clas=[-3:-1 1:6 0];
...
for k=inds
    fn=char(lsmlist(k));
%         s=dir([fn(1:end-4) '*Hand.mat']);
    if(opt==0)
        inf=[fn(1:end-4) 'SliceDataSF_Hand.mat'];
    else
        inf=[fn(1:end-4) 'SliceDataSFVis_Hand.mat'];
    end
    if(exist('infs'))
        infs=strvcat(infs,inf);
    else
        infs=inf;
    end
    smallf=[inf(1:end-4) 'Small.mat'];
    if(isfile(smallf))
        load(smallf)
    else
        [dat]=tempSmallFiles(inf,smallf);
    end
...
clustf=GetDataToCluster(indsi,lsmlist);
...
function[dat]=tempSmallFiles(inf,smallf)
...
function AutoClass(inds,lsmlist,opt)
if(nargin<3) 
    % opt=2 is auto-classification based on classified patches and blind hand
    % thresholds 
    opt=2;     
end;
% sfn={'1';'2';'3a';'4a'};
...
for i=inds
    ...
    % do auto-classification based on classified patches and blind
    % thresholds. Option 3 does it on class'd patches and visible threshes
    TextureSlices(f1,opt);    
%     TextureWholeSlice(f1,opt);
end

% if opt = 1 do classification on red channel (default 0)
function PickThreshesVis(inds,lsmlist,opt)
if(nargin<3) 
    opt=0; 
end;
for i=inds
    f1=char(lsmlist(i));
    GetStdThreshVis(f1,opt);
end
...
if(opt==1)
    ...
    fno2=[fn(1:end-4) '_StdThreshVisRed.mat']
    maxf=[fn(1:end-4) '_MaxesRed.mat'];
else
    fno=[fn(1:end-4) '_sl1_Mask.mat'];
    fno2=[fn(1:end-4) '_StdThreshVis.mat'];
...
% gets the threshold blndly by randomising the value inbetween slices
% this  means the previous file cannot be loaded hence worries on
% overwriting
...
function[thresh]=stdThresh(im,thresh,axlim,patches,numpat,maxt,opt)

if(nargin<7)
    opt=0;  % do threshold blind
end
...
while 1
    ...
    if(opt==1)
        % visibly do the threshold
        title([' threshold, currently ' int2str(thresh)]);
        x=input(['enter threshold, currently ' int2str(thresh) '. Return if ok:  ']);
        if(isempty(x))
            break;
        else
            thresh=max(0,x);
...
for i=inds% 1];
    ...
    %     im=tiffread29(s(i).name,11);
    %     subplot(1,2,1),imagesc(cell2mat(im.data(1)));
    %     subplot(1,2,2),imagesc(cell2mat(im.data(2)));
    if(opt==2)  % vecadretina stuff
        if(isequal(fn(1:3),'63X'))
            [vecadCh,vessCh]=deal(2,4);
...
%             save([fn(1:end-4) 'SliceDataSF_Hand_Red_ThAutoAdj.mat'],'dat')
        ...
        [m,s,n]=PlotRedVals(dat);%,fn(1:end-4));
        mea=[mea;m];
        da=[da dat];
        ns=[ns;n];
        pcn=[pcn; 100*n(1:6)/sum(n(1:6))];
        nm=n(1:6)+0.5*n([6 5 4 4:6]);
        nms=[nms;nm]
        pcn_mixed=[pcn_mixed;nm*100/sum(nm)];
    end
    save temp_allreddata da mea ns nms pcn* 
%     load temp_allreddata 
%     figure(j+1)%+rthresh)
%     PlotRedVals(da)
end
...
function[pco,spco,n]=PlotRedVals(dat)
% clas=[-3:6 10 11];
% pos=[-3:-1 9 1:3 5:7]
% clas=[-3:6];
% pos=[-3:-1 5 1:3 7:9]
...
ts=[];
% figure(14),
BarErrorBar(x',pco',spco'),title('% red over thresh in each class'),axis tight
...
function TextureWholeSlice(fn,opt)
...
for k=1%2:4
    ...
    if(opt==5)
        outf=[fn(1:end-4) 'WholeSliceDataSF_Hand_Red.mat']
    else
        outf=[fn(1:end-4) 'WholeSliceDataSF_Hand.mat']
    end
    dat=[];
        ...
        if(~isfile(fno))
            ...
            if(isequal(mask,-1))
                ...
                elseif(opt==2)
                    ...
                    da=AnalysePatch(pim,mim,[th1 sth2],thfs(k),sfs(k),-1,0);
                    da.fn_mask=fno;
                    da.fn_im=fn;
                    da.sl=sl;
                    da.th1=th1;
...
if(opt==1)
    sfs=[1000:500:3000];
elseif(opt==2)
    % this is the file for 'blind' object threshold picking
%     load(['HandThresholds\' fn(1:end-4) '_StdThreshV3.mat']);
    % file used to be read from HandThresholds to avoid overwritng. Now
    % just copy it into there so it's safe
    load([fn(1:end-4) '_StdThreshV3.mat']);
elseif(opt==3)
    % this is the file for visible object threshold picking
    load([fn(1:end-4) '_StdThreshVis.mat']);
end
...
for k=1%2:4
    ...
    elseif(opt==3)
        outf=[fn(1:end-4) 'SliceDataSFVis_Hand.mat']
    else
...
out.g_std=MyPrctile(vsim(vsim>0),[50 25 75])';
