function varargout = ellipseCov(V,me,show,p,c,x,y)
% ELLIPSE calculates an ellipse that fits the data using principal component analysis
%
% edited by me from ellipse to use a covariance matrix V
% 
% [area,axes,angles,ellip] = ellipse(V,'show',p)
% INPUTS:
...
if exist('p','var') && ~isempty(p) && p~=.95
    if exist('raylinv.m','file')==2
...
ellip = vec*invp*sqrt(val)*[cos(t); sin(t)] + repmat(me',1,100);
ellip = ellip';
axes  = axes';

% plot:
% used to pass in x and y if plotting
% Now passing in the mean
if exist('show') && ~isempty(show)
   if(nargin<5)
       c='r';
   end
   m = [me', me'];  %[mean(x) mean(x); mean(y) mean(y)];
   ax = [cos(angles); sin(angles)].*[axes; axes] + m;
%    figure
   plot(ellip(:,1),ellip(:,2),c,'linewidth',2)
   hold on
   plot([ax(1,:); 2*m(1,:)-ax(1,:)],[ax(2,:); 2*m(2,:)-ax(2,:)],[c '--'],'linewidth',2)
   if(nargin>5)
       plot(x,y,[c 'o'])
       plot(x([1 end]),y([1 end]),'>k','linewidth',3)
   end
